...
class TestDataGenerator:
    ...
    @staticmethod
    def create_point_annotations(frame_idx=0, height=480, width=640):
        """創建點擊點標註"""
        # 物件 1: 圓形中心
        obj1_center = [150 + frame_idx * 30, 200 + frame_idx * 10]
        
        # 物件 2: 矩形中心
        obj2_center = [400 - frame_idx * 20 + 30, 300 + frame_idx * 15 + 40]
        
        return {
            1: {"points": [obj1_center], "labels": [1]},
            2: {"points": [obj2_center], "labels": [1]}
        }


class TestSAM2WrapperDataStructures:
    """測試資料結構和枚舉"""
    
    def test_model_type_enum(self):
        """測試 SAM2ModelType 枚舉"""
        # 測試枚舉值
        assert SAM2ModelType.TINY.value == "sam2_hiera_tiny"
        assert SAM2ModelType.SMALL.value == "sam2_hiera_small"
        assert SAM2ModelType.BASE_PLUS.value == "sam2_hiera_base_plus"
        assert SAM2ModelType.LARGE.value == "sam2_hiera_large"
        
        # 測試字符串轉換
        assert SAM2ModelType("sam2_hiera_tiny") == SAM2ModelType.TINY
        
        # 測試無效類型
        with pytest.raises(ValueError):
            SAM2ModelType("invalid_model")
...
class TestSAM2WrapperInitialization:
    ...
    @pytest.mark.parametrize("model_type", [
        SAM2ModelType.TINY,  # 使用最小的模型進行測試
    ])
    @skip_on_ci
    def test_wrapper_initialization_with_models(self, model_type):
        ...
        try:
            ...
            pytest.skip(f"模型初始化失敗: {e}")
    
    def test_device_selection(self):
        """測試設備選擇邏輯"""
        # 測試指定 CPU
        try:
            tracker = SAM2Wrapper(model_type=SAM2ModelType.TINY, device='cpu')
        ...
        # 測試自動選擇
        try:
            tracker = SAM2Wrapper(model_type=SAM2ModelType.TINY, device=None)
...
class TestSAM2WrapperTracking:
    ...
    @skip_on_ci
    def test_video_tracker_initialization(self, tracker):
        """測試視頻追蹤器初始化"""
        frames = TestDataGenerator.create_sample_frames(3)
        
        # 初始化視頻追蹤器
        tracker.init_video_tracker(frames)
        
        assert tracker.video_initialized == True
        assert len(tracker.video_frames) == 3
        assert hasattr(tracker, 'inference_state')
    
    @skip_on_ci
    def test_add_object_with_points(self, tracker):
        """測試使用點擊點添加物件"""
        frames = TestDataGenerator.create_sample_frames(3)
        ...
        labels = [1, 1]
        obj_id = tracker.add_object_with_points(0, points, labels)
        
        assert obj_id == 1
        assert obj_id in tracker.active_objects
        assert tracker._next_object_id == 2
    
    @skip_on_ci
    def test_add_object_with_mask(self, tracker):
        """測試使用遮罩添加物件"""
        frames = TestDataGenerator.create_sample_frames(3)
        ...
        mask[200:240, 150:190] = 1
        
        # 添加物件
        obj_id = tracker.add_object_with_mask(0, mask)
        
        assert obj_id == 1
        assert obj_id in tracker.active_objects
    
    @skip_on_ci
    def test_video_tracking(self, tracker):
        """測試視頻追蹤"""
        ...
        # 添加多個物件
        annotations = TestDataGenerator.create_point_annotations(0)
        for obj_id, annotation in annotations.items():
            tracker.add_object_with_points(
                0, annotation["points"], annotation["labels"], obj_id=obj_id
            )
    ...
    def test_tracking_without_initialization(self, tracker):
        """測試未初始化時追蹤的錯誤處理"""
        with pytest.raises(RuntimeError, match="必須先調用 init_video_tracker"):
            tracker.add_object_with_points(0, [[100, 100]], [1])
    ...
    def test_restart_functionality(self, tracker):
        ...
        # 設置初始狀態
        tracker.init_video_tracker(frames)
        tracker.add_object_with_points(0, [[150, 200]], [1])
        ...
        with patch('urllib.request.urlopen') as mock_urlopen:
            ...
            try:
                ...
                if not SAM2_AVAILABLE:
                    ...
                    # 測試檔案不存在時的行為
                    try:
                        wrapper = SAM2Wrapper(checkpoint_path=str(test_file))
                    except Exception:
                        # 預期會失敗，因為檔案不存在
                        pass
            except Exception:
                pass  # 這是預期的，因為我們在模擬環境中
...
class TestSAM2WrapperPerformance:
    ...
    def test_tracking_speed(self, tracker):
        ...
        # 初始化並添加物件
        tracker.init_video_tracker(frames)
        tracker.add_object_with_points(0, [[120, 150]], [1])
        ...
        logging.info(f"追蹤速度: {fps:.2f} FPS (序列處理)")
        
        # 基本性能要求（至少能完成追蹤）
        assert len(tracks) > 0
        assert elapsed_time < 60  # 不應該超過1分鐘
    
    def test_memory_usage(self, tracker):
        """測試記憶體使用"""
        frames = TestDataGenerator.create_sample_frames(20, height=240, width=320)
...
class TestSAM2WrapperVisualization:
    ...
    def output_dir(self):
        ...
        # 在 tmp 目錄下創建唯一的測試目錄
        temp_dir = tempfile.mkdtemp(prefix="sam2_test_", dir=str(tmp_dir))
    ...
    def test_synthetic_video_tracking(self, output_dir):
        ...
        # 生成測試影片
        num_frames = 30
        frames = self._generate_synthetic_video_frames(num_frames=num_frames)
        
        # 保存原始影片
        if VIDEO_IO_AVAILABLE:
            original_video_path = output_dir / "sam2_synthetic_original.mp4"
        ...
        ax.imshow(frames[0])
        ax.set_title('First Frame with Click Points')
        
        # 標記預定的點擊位置 (修正後的正確位置)
        click_points = [
            (470, 240),              # 圓形初始位置: center_x=320+150, center_y=240+0
            (230, 380),              # 矩形初始位置: 矩形中心
            (100, 100)               # 三角形初始位置: tri_center_x=100, tri_center_y=100
        ]
        ...
        first_frame_path = output_dir / "sam2_synthetic_first_frame_clicks.png"
        plt.savefig(first_frame_path, dpi=150, bbox_inches='tight')
        plt.close()
        print(f"第一幀點擊點已保存: {first_frame_path}")
        
        # 初始化 SAM2 追蹤器
        try:
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            tracker = SAM2Wrapper(
                model_type=SAM2ModelType.TINY,  # 使用較小的模型進行測試
                device=device
            )
            print(f"SAM2 追蹤器初始化成功 (device: {device})")
        except (FileNotFoundError, RuntimeError) as e:
            pytest.skip(f"SAM2 模型初始化失敗: {e}")
        
        # 初始化視頻追蹤器
        tracker.init_video_tracker(frames)
        print(f"視頻追蹤器初始化完成，載入 {len(frames)} 幀")
        ...
        # 保存追蹤結果可視化
        self._save_tracking_visualization(frames, tracks, output_dir, "sam2_synthetic_tracking")
        
        # 保存追蹤指標
        metrics = self._save_tracking_metrics(tracks, output_dir, "sam2_synthetic_tracking")
...
def run_manual_visualization_tests():
    ...
    tester = TestSAM2WrapperVisualization()
    ...
    temp_dir = tempfile.mkdtemp(prefix="sam2_manual_test_", dir=str(tmp_dir))
    output_dir = Path(temp_dir)
    ...
    try:
        ...
        print(f"SAM2 合成影片測試成功: {result1}")
    except Exception as e:
        print(f"SAM2 合成影片測試失敗: {e}")
        import traceback
        traceback.print_exc()
